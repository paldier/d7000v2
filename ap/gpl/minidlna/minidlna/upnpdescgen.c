/* $Id: upnpdescgen.c,v 1.15 2009/07/17 22:54:31 jmaggard Exp $ */
/* MiniUPnP project
...
static const struct XMLElt rootDesc[] =
{
	...
	{"/presentationURL", device_url},//presentationurl},	/* recommended */
                   ...
                   const struct XMLElt * p)
{
	...
	for(;;)
	{
		...
		if(eltname[0] == '/')
		{
			...
			str = strcat_char(str, len, tmplen, '>');
			/* Foxconn, add by MJ. */
			if(!strcmp(p[i].eltname+1, "controlURL")||
			   !strcmp(p[i].eltname+1, "eventSubURL")||
			   !strcmp(p[i].eltname+1, "SCPDURL"))
			{
				sprintf(tmp, "%s%s", presentationurl, p[i].data);
