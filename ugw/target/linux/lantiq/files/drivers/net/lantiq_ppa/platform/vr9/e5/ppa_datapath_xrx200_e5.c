...
#endif
#define xTM_MODE_TO_DSL                     	SB_BUFFER(2, 0x7DC4)
#define xTM_BC0_SIZE_TO_DSL                     SB_BUFFER(2, 0x7DC5)
...
static int eth_qos_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    len =  skb->len;//switch will do the padding
...
static INLINE int eth_xmit(struct sk_buff *skb, int port, int ch, int spid, int class)
{
    ...
    //len = skb->len <= ETH_MIN_TX_PACKET_LENGTH ? ETH_MIN_TX_PACKET_LENGTH : skb->len;
    len = skb->len;//switch will do padding
...
#if defined(CONFIG_LTQ_MINI_JUMBO_FRAME_SUPPORT)
		BOND_CONF->max_frag_size	= 1604;
        *PDMA_RX_MAX_LEN_REG        = 0x02040660;
#else
...
static INLINE void init_pdma(void)
{
    ...
    *PDMA_RX_MAX_LEN_REG= g_dsl_bonding ? 0x02040204 : 0x02040660;
...
#define DSL_US_BC0 0x28
#define DSL_DS_BC0 0x80

static INLINE void init_dsl_hw(void)
{
	uint16_t us_bc0 = DSL_US_BC0 * DSL_PAGE_SIZE;
	uint16_t ds_bc0 = DSL_DS_BC0 * DSL_PAGE_SIZE;
	volatile unsigned int *ppe_tc_bc0_size = xTM_BC0_SIZE_TO_DSL;

	//Increase DS ReTX buffer : 0x00010040, DSL_DS_BC0
    IFX_REG_W32(0x00010080, SFSM_CFG0);
    IFX_REG_W32(0x00010000, SFSM_CFG1);
    ...
    IFX_REG_W32(0x00030028, FFSM_CFG0);         //  Force_idle, DSL_DS_BC0
