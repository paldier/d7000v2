...
typedef struct ArgParameter_s {
	...
	char sArgValue[MAX_LEN_OBJNAME];	/*!< String to hold value of User given values as we see in Command line */
...
*/
static int cli_parseDataFromXML(clish_context_t * clish_context, char *pcServiceName, MainOper * peMainOp, SubOper * peSubOper, InputData_t * pxInData);
...
/*!	\brief This function is top-level function for various configuration commands. It further branches to
	...
	\return UGW_SUCCESS / UGW_FAILURE 
*/
static int cli_configureCommandHandler(SubOper eSubOp, InputData_t * pxInData);
...
/*!	\brief Reads command and determines if it is a Show command or Configure command. If Configure command,
	...
	\return UGW_SUCCESS / UGW_FAILURE 
*/
static int cli_analyzeCommand(char *pcCmd, MainOper * peMainOp, SubOper * peSubOper, char *pcService);
