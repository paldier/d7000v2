...
CLISH_PLUGIN_SYM(cli_cmdHandler)
{
	...
	SubOper eSubOper = OBJOPT_MODIFY;
...
***********************************************************************************************************/
int cli_parseDataFromXML(clish_context_t * clish_context, char *pcServiceName, MainOper * peMainOp, SubOper * peSubOper, InputData_t * pxInData)
{
	int nRet = UGW_SUCCESS;
...
***********************************************************************************************************/
int cli_analyzeCommand(char *pcCmd, MainOper * peMainOp, SubOper * peSubOper, char *pcService)
{
	int nRet = UGW_SUCCESS;
...
int cli_parseCmdLineArguments(clish_pargv_t * pArgv, InputData_t * pxInData)
{
	...
	for (nArg = 0; nArg < pArgv->pargc; nArg++) {
		...
		strncpy(xLocalArg.sArgValue, pArgv->pargv[nArg]->value, MAX_LEN_OBJNAME);
		...
		if (nArg == 0) {
			...
			strncpy(pxInData->xFilterParam.sArgValue, xLocalArg.sArgValue, MAX_LEN_OBJNAME);
...
***********************************************************************************************************/
int cli_configureCommandHandler(SubOper eSubOp, InputData_t * pxInData)
{
	int nRet = UGW_SUCCESS;
		...
		if (cli_matchInstancedStrings(pxLinkTrav->sLinkedObj, pcInitRefObj)) {
			...
			while (pxBackTrav != NULL) {
				...
				if (pxGetObjList == NULL) {
					nRet = UGW_FAILURE;
					LOGF_LOG_DEBUG("Unable to get pointer to object's of type %s\n", pxBackTrav->sLinkingObjPreAsterisk);
					goto end;
				}
